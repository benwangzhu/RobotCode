...
.program pg.mix_cfg_()
    ...
    if BoxOrConv == true then 
        ...
        ; 码垛框 1 配置数据
        placeUpDst[0]           = 300.0                 ; 机器人每次进框和出框时的位置
        PlaceMaxX[0]            = 2000.0                ; -- 码垛框位置极限定义, 存在导轨轴时，值需要包含导轨值
        PlaceMinX[0]            = -2000.0               
        PlaceMaxY[0]            = 2000.0
        PlaceMinY[0]            = -2000.0                                                             ; --
        if NumAxis == 7 then
            
            placeE7Pos[0]       = 0.0                   ; 码框时的导轨轴的位置
        end
    ...
    ; 工具坐标系定义
    point MagTool               = trans(0, 0, 0, 0, 0, 180)
.end
        ...
        if sig(MixAckReq) == true then
            ...
            value GET_DATA:
                ...
                if (MixStatus == 0) and (BusInput[DI_FINISHED] == false) then
                    ...
                    if NumAxis == 7 then
                        point/7 VsnPickPos = VsnPictPos
                    end

                    call pg.safety_chk_(&VsnPickPos, NumAxis, PkSafetyMinX, PkSafetyMaxX, PkSafetyMinY, PkSafetyMaxY, MixStatus)

                    if (BoxOrConv == true) and (MixStatus == 0) then  

                        point VsnDropPos = trans(VsnPlPosn[0], VsnPlPosn[1], VsnPlPosn[2], VsnPlPosn[3], VsnPlPosn[4], VsnPlPosn[5])

                        if NumAxis == 7 then
                            point/7 VsnDropPos = trans(0, 0, 0, 0, 0, 0, PlaceE7Pos[VsnPlaceId - 1])
                        end
...
.program pg.assig_data_()
    ...
    point .SafePos = trans(.PkPosAry[0], .PkPosAry[1], .PkPosAry[2], .PkPosAry[3], .PkPosAry[4], .PkPosAry[5], .PkPosAry[6])

    point/X OrentPkPos = .SafePos

    if NumAxis == 7 then  
        point/7 OrentPkPos = ThisPkPos
    end

    if .PkPosAry[6] > 1000.0 then  
    ...
    if BoxOrConv then
        ...
        if NumAxis == 7 then  
            ...
            point/7 NewDropPos = trans(0, 0, 0, 0, 0, 0, PlaceE7Pos[VsnPlaceId - 1])
            point/7 NewDropPosUp = trans(0, 0, 0, 0, 0, 0, PlaceE7Pos[VsnPlaceId - 1])
        end
        ...
        if NumAxis == 7 then  
            point/7 NewDropPos = ConvDropPos[VsnPlaceId - 1]
        end
    end
.end
...
.program pg.move_wpos_()
    ...
    point/Z SafeWaitPos = orentpkpos
    ;point/7 SafeWaitPos = here
...
.program pg.mov_pick_()
    ...
    lmove shift(ThisPkPos by 0, 0, 50)
...
.program pg.go_home()
    ...
    if .P[3] < 50 then  
        ...
        point .npos =  trans(.P[1], .P[2], 50, .P[4], .P[5], .P[6], .P[7])
        lmove .npos
    end
    ...
    if distance(here, .Home1) < 10 goto pass2
...
pass1:
    ...
    twait 0.5
    decompose .N[1] = #here
    jmove #ppoint(.N[1], .N[2], .N[3], .N[4], .N[5], .N[6], .Home[7])
    break
    jmove #ppoint(.N[1], .Home[2], .Home[3], .N[4], .Home[5], .N[6], .Home[7])
    break
    jmove #ppoint(.N[1], .Home[2], .Home[3], .Home[4], .Home[5], .Home[6], .Home[7])
    break
...
.program pg.zone_(.&ThisPos, .ZoneData1, .ZoneData2)

    decompose .ThisPosAry[1] = .ThisPos
    .ThisPosAry[1] = .ThisPosAry[1] + .ThisPosAry[7]

    if .ThisPosAry[1] > .ZoneData1 then  
        call lib.enter_zone_(2)             ; With Abb
        return
    end

    if .ThisPosAry[1] < .ZoneData2 then  
        call lib.enter_zone_(1)             ; With Yaskawa
        return  
    end
